...
createApp({
  ...
  computed: {
    ...
    totalPrice() {
      return this.categories.reduce((sum, c) => {
        ...
        return sum + (item ? this.estimatePrice(item) : 0);
      }, 0);
    },
    cartItems() {
    ...
    pickerItems() {
      ...
      return source.filter((x) => {
        ...
        const byTag = !this.pickerTag || this.matchesTag(x, this.pickerTag) || meta.includes(this.pickerTag.toLowerCase()) || name.includes(this.pickerTag.toLowerCase());
        return byText && byTag;
      });
    },
    pickerFilters() {
  ...
  methods: {
    ...
    componentNameById(id) {
      const item = this.components.find((x) => Number(x.id) === Number(id));
    ...
    estimatePrice(item) {
      ...
      const base = Number(item.perf_score || 100);
      return Math.max(59, Math.round(base * 1.35));
    },
    itemThumb(item) {
    ...
    async validateBuild() {
      try {
        const result = await this.api('/api/components/validate', {
          method: 'POST',
          body: JSON.stringify(this.buildPayload())
        });
        if (!result.issues.length) {
          this.statusText = 'Сборка совместима.';
          this.statusClass = 'ok';
          return;
        }
        this.statusText = result.issues.map((x) => `[${x.severity}] ${x.message}`).join('\n');
        this.statusClass = result.compatible ? 'ok' : 'error';
      } catch (e) {
    ...
    async saveConfig() {
      const title = window.prompt('Название конфигурации');
      if (!title) return;
      await this.api('/api/configurations', {
        method: 'POST',
        body: JSON.stringify({ title, config: this.buildPayload() })
      });
      this.statusText = 'Конфигурация сохранена.';
      this.statusClass = 'ok';
    ...
    toggleCompare(itemId) {
      ...
      if (this.compareList.length >= 3) {
        this.statusText = 'Для сравнения можно выбрать максимум 3 товара.';
        this.statusClass = 'error';
        return;
      }
    ...
    async loadSaved() {
      const data = await this.api('/api/configurations');
      this.savedConfigs = data.configurations;
    },
    applyConfig(cfg) {
      const next = { CPU: null, MOTHERBOARD: null, RAM: null, GPU: null, PSU: null, STORAGE: null, COOLER: null };
      Object.entries(cfg.config_json || {}).forEach(([category, id]) => {
        if (Object.prototype.hasOwnProperty.call(next, category)) {
          next[category] = Number(id);
        }
      });
      this.selected = next;
      this.statusText = `Конфигурация "${cfg.title}" загружена.`;
      this.statusClass = 'ok';
    },
    async editConfig(cfg) {
      const nextTitle = window.prompt('Новое название конфигурации', cfg.title);
      if (!nextTitle || nextTitle.trim().length < 2) return;
      await this.api(`/api/configurations/${cfg.id}`, {
        method: 'PUT',
        body: JSON.stringify({ title: nextTitle.trim() })
      });
      this.statusText = 'Конфигурация обновлена.';
      this.statusClass = 'ok';
      await this.loadSaved();
    },
    async deleteConfig(id) {
      const ok = window.confirm('Удалить конфигурацию?');
      if (!ok) return;
      await this.api(`/api/configurations/${id}`, { method: 'DELETE' });
      this.statusText = 'Конфигурация удалена.';
      this.statusClass = 'ok';
      await this.loadSaved();
    },
    logout() {
    ...
    closeWizard() {
      ...
      this.wizardStep = 1;
      localStorage.setItem('builder_wizard_seen', '1');
    }
  },
  async mounted() {
    ...
    await this.loadSaved();
    this.showWizard = localStorage.getItem('builder_wizard_seen') !== '1';
  }
}).mount('#builderApp');
